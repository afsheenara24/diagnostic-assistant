...
with col1:   
    if st.session_state['lang_changed'] and 'gender_1' in st.session_state:     
        del st.session_state['gender_1']

    # If gender_1 not set or invalid, reset it
    if 'gender_1' not in st.session_state or st.session_state['gender_1'] not in genders_list:
        st.session_state['gender_1'] = genders_list[0]
...
# Pregnancy
# ⚠️ FIX: use 'gender_1' (not 'gender') because that's your actual radio key
if st.session_state['gender_1'] == transl[lang]['male']:
    st.session_state.disabled = True
    # Auto-set pregnant = "No" (in the current language)
    st.session_state['pregnant'] = transl[lang]['no']
else: 
    st.session_state.disabled = False
...
with col3: 
    ...
    # If not set or invalid, reset
    if 'pregnant' not in st.session_state or st.session_state['pregnant'] not in pregnant_list:
        st.session_state['pregnant'] = pregnant_list[0]

    st.radio("**{}**".format(transl[lang]['pregnant']),
             pregnant_list,
